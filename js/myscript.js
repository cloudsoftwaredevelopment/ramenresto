...
document.getElementById('orderSummaryItems');
        ...
        submitLogin.addEventListener('click', () => {
            const selectedRole =
document.querySelector('.role-btn.selected');
            if (selectedRole) {
                ...
                if (currentUserRole === 'manager' || currentUserRole ===
'supervisor') {
...
btn.classList.remove('selected'));
        ...
        function updateCart() {
            ...
            cart.forEach((item, index) => {
                const itemTotal = item.price + item.addons.reduce((sum,
...
=> addon.name).join(', ')}</div>
                    ...
                    <button class="remove-item" data-index="${index}"><i
class="fas fa-times"></i></button>
            ...
            itemCount.textContent = `${cart.length} ${cart.length === 1 ?
'item' : 'items'}`;
...
addon) => sum + addon.price, 0);
                ...
                summaryItem.innerHTML = `
                    <span>${item.name} ${item.addons.length ? `+
${item.addons.map(a => a.name).join(', ')}` : ''}</span>
